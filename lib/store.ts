...
function redisStore(redis: Redis): Store {
  let persisted = false;
  // 투표·응답은 삭제하기 전까지 계속 보관. 예전에 걸어 둔 만료 시각을 처음 한 번 모두 해제
  const persistAll = async () => {
    if (persisted) return;
    if (!(await redis.get(NO_EXPIRE))) {
  ...
  return {
    ...
    async listPolls(limit) {
      ...
      for (let i = 0; i < ids.length; i++) {
        const raw = res[i * 2];
        if (!raw) {
          expired.push(ids[i]); // 본문이 없어진 투표는 목록에서도 정리
          continue;
        }
